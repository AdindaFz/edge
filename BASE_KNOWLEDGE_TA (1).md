...
> **⚠️ CATATAN REVISI (April 2026):** Dokumen ini telah diperbarui berdasarkan diskusi keputusan final. Lihat Bagian 9 untuk detail lengkap perubahan terminologi dan framing penulisan.
...
9. [Update Keputusan Final & Terminologi (April 2026)](#9-update-keputusan-final--terminologi-april-2026)
...
### Pendekatan Utama: Hybrid Tabu–Diffusion Scheduling
...
| Layer | Fungsi | Metode yang Diuji |
|-------|--------|-------------------|
| **Global Optimizer** | Eksplorasi solusi scheduling global (search space luas) | Tabu Search, PSO, BFO |
| **Local Balancing Mechanism** | Balancing beban antar node setelah keputusan global | CPM, Diffusion, None |

> **Terminologi resmi (wajib konsisten):**
> - Global Optimizer → **Tabu Search**
> - Local Balancing Mechanism → **Diffusion**
> - Nama metode final → **Hybrid Tabu–Diffusion Scheduling**
...
Topologi grid mendefinisikan **adjacency** antar node (atas, bawah, kiri, kanan), yang digunakan oleh **Diffusion Local Balancing Mechanism** untuk menentukan arah migrasi task.

### 3.2 Konfigurasi Node (Heterogeneous Edge System Model)

> **Posisi dalam penulisan**: Heterogenitas node adalah **karakteristik lingkungan eksperimen (System Model)**, bukan novelty utama. Dibahas di BAB 3.3 sebagai asumsi sistem, bukan kontribusi algoritmik.

Heterogenitas node mencerminkan kondisi nyata edge computing di mana setiap node memiliki kemampuan komputasi yang berbeda-beda, menyebabkan **load imbalance** secara natural jika tidak dikelola dengan baik.

9 node dikonfigurasi dengan kapasitas **heterogen** (tidak seragam), disampling dari dataset mesin nyata Google Cluster Trace dan diskalakan dengan faktor heterogenitas:
...
**Implikasi heterogenitas terhadap sistem:**
- Perbedaan `cpu_caps` dan `mem_caps` antar node → utilization tiap node berbeda untuk task yang sama
- Node dengan kapasitas kecil lebih cepat overloaded → sumber load imbalance
- Algoritma yang tidak aware terhadap kapasitas (Round-Robin) menghasilkan distribusi yang tidak efisien
...
#### A. HETEROGENEITY-AWARE TABU SEARCH (Global Optimizer)
...
**Bukti Heterogeneity-Aware (dari kode):**

*A. Candidate move generation mempertimbangkan kapasitas node:*
```python
# Node selection weighted by utilization (heterogeneity-aware)
node_weights = 1.0 / (1.0 + cpu_util)
# cpu_util = cpu_used / cpu_caps
# Karena cpu_caps berbeda tiap node → keputusan move aware terhadap kapasitas heterogen
```

*B. Objective function mengevaluasi cost dengan cpu_caps dan mem_caps nyata:*
```python
# Fitness dihitung menggunakan kapasitas riil masing-masing node
energy_of_configuration(cpu_caps, mem_caps, ...)
latency_of_configuration(cpu_caps, mem_caps, ...)
# → heterogeneity-aware cost evaluation
```

> **Framing penulisan yang benar:**
> ✅ "Heterogeneity-aware Tabu Search digunakan sebagai Global Optimizer, sedangkan Diffusion digunakan sebagai Local Balancing Mechanism."
> ❌ "Tabu Search standar digunakan sebagai global optimizer"
...
#### E. DIFFUSION LOCAL BALANCING MECHANISM ← TERBAIK
...
**Diffusion juga bersifat Heterogeneity-Aware (Power-Aware):**
```python
# Task migration mempertimbangkan utilization relatif terhadap kapasitas node
neighbor_util = cpu_used[neighbor] / cpu_caps[neighbor]
# Serta energy neighbor saat menerima task baru
energy_neighbor = node_power * neighbor_util
```
Ini berarti Diffusion tidak sekadar menyeimbangkan jumlah task, tetapi mempertimbangkan **kapasitas riil** dan **konsumsi daya** tiap node → **power-aware heterogeneous local balancing**.
...
- Diffusion **power-aware** dan **capacity-aware** terhadap heterogenitas node

---

## 5. METODE YANG DIBANDINGKAN

| No | Metode | Global Optimizer | Local Balancing Mechanism | Keterangan |
|----|--------|-----------------|--------------------------|------------|
| 1 | **TABU + NONE** | Tabu Search | - | Baseline Tabu tanpa local balancing |
| 2 | **TABU + CPM** | Tabu Search | CPM | Tabu + Cellular Potts Model |
| 3 | **TABU + DIFFUSION** ⭐ | Tabu Search | Diffusion | **TERBAIK — Hybrid Tabu–Diffusion** |
| 4 | **PSO + CPM** | PSO | CPM | Swarm-based + CPM |
| 5 | **BFO + DIFFUSION** | BFO | Diffusion | Bio-inspired + Diffusion |

### Kenapa TABU + DIFFUSION Terbaik?

1. **Heterogeneity-aware Tabu Search** efektif menghindari revisit solusi buruk (memory-based), cocok untuk discrete assignment problem, dan secara teknis mempertimbangkan kapasitas heterogen setiap node dalam candidate move generation maupun cost evaluation
2. **Diffusion Local Balancing Mechanism** secara fisik menyeimbangkan beban dengan cara yang natural, langsung mempengaruhi objective function, dan bersifat power-aware terhadap heterogenitas kapasitas node
3. Kombinasi keduanya saling melengkapi: Tabu menjelajah global search space, Diffusion merapikan distribusi beban secara lokal mengikuti topologi grid
...
NAMA RESMI    : Hybrid Tabu–Diffusion Scheduling
GLOBAL        : Heterogeneity-aware Tabu Search
LOCAL         : Diffusion Local Balancing Mechanism
HETEROGENEITY : System Model (bukan novelty utama)
```

---

## 9. UPDATE KEPUTUSAN FINAL & TERMINOLOGI (April 2026)

> Bagian ini merupakan **keputusan final** yang harus diterapkan secara konsisten di seluruh penulisan laporan. Update ini menggantikan interpretasi sebelumnya.

---

### 9.1 Posisi Heterogeneous Nodes

**Keputusan final:** Heterogenitas node diposisikan sebagai **System Model**, bukan novelty utama.

**Implikasi penulisan:**
- Heterogenitas node dibahas di **BAB 3.3 Pemodelan Sistem** sebagai karakteristik lingkungan eksperimen
- Digunakan untuk menjelaskan: perbedaan CPU capacity, perbedaan memory capacity, dan load imbalance yang timbul akibat resource yang tidak seragam
- **Tidak diklaim sebagai novelty utama**, karena merupakan asumsi/karakteristik sistem, bukan kontribusi algoritmik

---

### 9.2 Terminologi Resmi (Locked — Wajib Konsisten)

Gunakan istilah berikut secara **konsisten di seluruh dokumen laporan**:

| Istilah Resmi | Hindari |
|---------------|---------|
| **Hybrid Tabu–Diffusion Scheduling** | Hybrid optimization, hybrid method |
| **Global Optimizer** → Tabu Search | Global search, main optimizer |
| **Local Balancing Mechanism** → Diffusion | Local optimizer ❌, local search ❌ |
| **Heterogeneous Edge System Model** | Heterogeneous nodes (tanpa konteks) |
| **Heterogeneity-aware Tabu Search** | Tabu Search standar |

---

### 9.3 Framing Penulisan Metode (Wajib Diterapkan)

**Cara menulis yang benar:**

✅ *"Pada penelitian ini diusulkan Hybrid Tabu–Diffusion Scheduling untuk optimasi task scheduling di edge computing. Heterogeneity-aware Tabu Search digunakan sebagai Global Optimizer untuk mengeksplorasi ruang solusi secara global, sedangkan Diffusion digunakan sebagai Local Balancing Mechanism untuk menyeimbangkan beban antar node berdasarkan topologi grid."*

❌ *"Tabu Search standar digunakan sebagai global optimizer dan Diffusion sebagai local optimizer."*

---

### 9.4 Verifikasi Teknis: Tabu Search bersifat Heterogeneity-Aware

Meskipun heterogenitas diposisikan sebagai system model, implementasi algoritma secara teknis **memang mempertimbangkan heterogenitas**. Ini penting untuk penulisan yang akurat secara teknis.

**Bukti dari kode:**

**A. Candidate move generation — weighted by utilization:**
```python
node_weights = 1.0 / (1.0 + cpu_util)
# cpu_util = cpu_used / cpu_caps
# cpu_caps berbeda tiap node → keputusan move aware kapasitas heterogen
```

**B. Objective function — heterogeneity-aware cost evaluation:**
```python
# Fitness menggunakan kapasitas riil masing-masing node
energy_of_configuration(cpu_caps, mem_caps, ...)
latency_of_configuration(cpu_caps, mem_caps, ...)
```

**C. Diffusion — power-aware heterogeneous local balancing:**
```python
# Migrasi task mempertimbangkan utilization relatif kapasitas node
neighbor_util = cpu_used[neighbor] / cpu_caps[neighbor]
energy_neighbor = node_power * neighbor_util
```

**Kesimpulan**: Meskipun tidak diklaim sebagai novelty utama, **secara teknis algoritma ini tidak generik** — ia beroperasi dengan full awareness terhadap kapasitas heterogen setiap node.

---

### 9.5 Novelty Utama (Final)

Kontribusi utama penelitian ini adalah:

1. **Hybrid optimization architecture:**
   - Heterogeneity-aware Tabu Search sebagai Global Optimizer
   - Diffusion sebagai Local Balancing Mechanism

2. **Dual-objective scheduling:**
   - Minimisasi energy consumption
   - Minimisasi latency
   - Secara simultan dalam satu objective function

3. **Two-level validation:**
   - Simulasi program (300 runs, CDF analysis)
   - Implementasi di VM nyata (VPS proof-of-concept)

> **Catatan**: Heterogeneous nodes = bagian system model. Bukan novelty, tetapi relevan secara teknis karena algoritma bersifat heterogeneity-aware.

---

*Dokumen ini dibuat sebagai acuan penyusunan Laporan Tugas Akhir.*
*Update terakhir: April 2026 — Revision: Terminologi & Framing Final*
