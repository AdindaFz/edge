...
def random_assignment(tasks, nodes, rng=None):
    ...
    sampled_nodes = rng.choice(node_ids, size=len(tasks), replace=True)

    for task, node in zip(tasks, sampled_nodes):
        assignments[task["task_id"]] = str(node)
...
def tabu_assignment(
    ...
    use_stagnation_diffusion = local_mode in {"diffusion", "stagnation_diffusion", "tabu_diffusion"}
