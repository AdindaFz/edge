...
def freeze_baseline_assignment(tasks, node_ids, seed=BASELINE_SEED):
    rng = np.random.default_rng(seed)
    baseline_node_probs = np.full(len(node_ids), 1.0 / len(node_ids), dtype=float)
...
def run_offline_experiment(
    ...
    baseline_assignment, baseline_indices = freeze_baseline_assignment(tasks, node_ids)
