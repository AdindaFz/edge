...
OUTPUT_PATH = Path("/home/adinda-central/edge-computing-system/outputs") / "main3_sweep_energy_first_100.json"

# More aggressive energy-first sweep than sweep_main3_fast.py.
ENERGY_WEIGHTS = [0.65, 0.75, 0.85]
HIGH_POWER_PENALTIES = [1.35, 2.25, 3.00]
VPS_PENALTY_SCALES = [0.35, 0.50, 0.70]
...
def rank_key(summary):
    ...
    energy_improved = energy_gain > 0
    return (
        0 if both_improved else 1,
        0 if energy_improved else 1,
        -energy_gain,
        -latency_gain,
...
def main():
    ...
    for idx in range(N_RANDOM_BASELINES):
        print("\n============================================================")
        print(f"RUN main3 energy-first random baseline {idx + 1}/{N_RANDOM_BASELINES}")
    ...
    print("\n================ MAIN3 ENERGY-FIRST SWEEP TABLE ================")
    print(
        f"{'ew':>6} {'hpp':>6} {'vps':>6} "
