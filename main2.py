...
def main():
    ...
    for idx in range(N_RANDOM_BASELINES):
        print(f"\n=== RANDOM RUN {idx + 1}/{N_RANDOM_BASELINES} (MAIN2) ===")
    ...
    res_tabu_legacy, _ = run_offline_experiment(
        tasks,
        "tabu_legacy",
        E_ref=e_ref,
        L_ref=l_ref,
    ...
    metrics_tabu_legacy = compute_metrics(res_tabu_legacy, tasks, NODE_RESOURCES)

    print("\n=== TABU LEGACY OBJECTIVE + REAL VPS EVALUATION ===")
    print_metrics(metrics_tabu_legacy)
    print_sample_results(res_tabu_legacy, "TABU LEGACY SAMPLE")

    print("\n=== COMPARISON: RANDOM vs TABU LEGACY ===")
    print_all_comparison_table(metrics_random, metrics_tabu_legacy, n_tasks=N_TASKS)
