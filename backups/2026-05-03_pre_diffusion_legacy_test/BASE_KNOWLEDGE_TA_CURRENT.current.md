...
Simulasi pada `Comparision.ipynb` memakai sistem edge heterogen berbentuk grid 3x3.
...
1. Diffusion mengevaluasi perpindahan lokal antar node tetangga.
2. Pada tuning terbaru, kandidat perpindahan dievaluasi menggunakan objective global energy-latency, bukan hanya local score.
3. Task hanya dipindahkan jika perpindahan tersebut menurunkan objective global melebihi ambang numerik `objective_epsilon`.
4. Pada simulasi, tetangga mengikuti grid 4-neighbor.
5. Pada VPS, adjacency saat ini semua node ke semua node, sehingga narasi topologi grid harus dibatasi pada fase simulasi.
6. Pada tuning terbaru, Diffusion tidak lagi dipanggil secara periodik setiap beberapa iterasi. Diffusion hanya dipicu ketika Tabu Search mengalami stagnasi, sehingga berfungsi sebagai mekanisme escape/refinement yang lebih konservatif dan selaras dengan implementasi VPS.
...
Catatan status: tabel hasil pada bagian ini berasal dari konfigurasi objective sebelumnya. Setelah bobot simulasi diselaraskan menjadi energy 0.5 dan latency 0.5, screening dan CDF perlu dijalankan ulang sebelum angka final digunakan dalam skripsi.
...
6. Energy model dan estimated real energy belum selalu selaras.
